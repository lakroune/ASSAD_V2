...
if (
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && (isset($_POST['id_animal_info']) or isset($_POST['id_animal_edit']))) {
        ...
        $info_animal = new Animal();
        if (!empty($_POST['id_animal_info'])) {
            $info = true;
            $info_animal->getAnimal($_POST['id_animal_info']);
        }
        if (!empty($_POST['id_animal_edit'])) {
            $edit = true;
            $info_animal->getAnimal($_POST['id_animal_edit']);
        }
    }
} else {
                    ...
                    <div class="bg-surface-light dark:bg-surface-dark p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h3 class="text-xl font-bold text-text-light dark:text-text-dark"><?= count(array_filter($array_animaux, fn($animal) => $animal->getTypeAlimentation() === 'Carnivore')) ?></h3>
                            </div>
                        </div>
                    </div>
                    <div class="bg-surface-light dark:bg-surface-dark p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h3 class="text-xl font-bold text-text-light dark:text-text-dark"><?= count(array_filter($array_animaux, fn($animal) => $animal->getTypeAlimentation() === 'herbivore')) ?></h3>
                            </div>
                        </div>
                    </div>
                    <div class="bg-surface-light dark:bg-surface-dark p-5 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h3 class="text-xl font-bold text-text-light dark:text-text-dark"><?= count(array_filter($array_animaux, fn($animal) => $animal->getTypeAlimentation() === 'omnivore')) ?></h3>
                            </div>
                        </div>
                    </div>
                </section>
                                        ...
                                        <td class="px-6 py-3">
                                            <div class="flex items-center gap-3">
                                                <div class="h-10 w-10 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                                                    <img alt="<?= ($animal->getNomAnimal()) ?>" class="h-full w-full object-cover"
                                                        src="<?= ($animal->getImageUrl()) ?>" />
                                                </div>
                                                <div class="flex flex-col">
                                                    <span class="font-bold text-text-light dark:text-text-dark"><?= ($animal->getNomAnimal()) ?></span>
                                                    <span class="text-xs text-text-secondary-light">ID: <?= ($animal->getIdAnimal()) ?></span>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-6 py-3">
                                            <span class="text-text-light dark:text-text-dark font-medium"><?= ($animal->getEspeceAnimal()) ?></span>
                                        </td>
                                        <td class="px-6 py-3">
                                        ...
                                        <td class="px-6 py-3 text-right">
                                            <div
                                                ...
                                                <form action="" method="POST" class="edit">
                                                    <input type="hidden" value="<?= $animal->getIdAnimal() ?>" name="id_animal_edit">
                                                    <button
                                                ...
                                                <form action="" method="POST" class="info">
                                                    <input type="hidden" value="<?= $animal->getIdAnimal() ?>" name="id_animal_info">
                                                    <button
                                                ...
                                                <form action="php/delete_animal.php" method="POST" class="delete">
                                                    <input type="hidden" value="<?= $animal->getIdAnimal() ?>" name="id_animal">
                                                    <button
                                                        type="button"
            ...
            <div class="bg-surface-light dark:bg-surface-dark w-full max-w-lg p-8 rounded-2xl shadow-2xl border border-primary/20">
                <div class="flex justify-between items-start mb-6">
                    <div class="h-20 w-20 rounded-full shadow-lg border-2 border-primary"
                        style="background-image: url('<?= ($info_animal->getImageUrl()) ?>'); background-size: cover; background-position: center;">
                    </div>
                    <div class="flex-1 ml-4">
                        <p class="text-primary font-bold text-xs uppercase tracking-widest">Fiche Animal</p>
                        <h2 class="text-3xl font-black"><?= ($info_animal->getNomAnimal()) ?></h2>
                    </div>
                ...
                <div class="grid grid-cols-2 gap-4 mb-6">
                    <div class="p-3 bg-gray-50 dark:bg-background-dark rounded-xl">
                        ...
                        <p class="font-bold"><?= ($info_animal->getEspeceAnimal()) ?></p>
                    </div>
                    <div class="p-3 bg-gray-50 dark:bg-background-dark rounded-xl">
                        <p class="text-xs text-text-secondary-light">Alimentation</p>
                        <p class="font-bold"><?= ($info_animal->getTypeAlimentation()) ?></p>
                    </div>
                    <div class="p-3 bg-gray-50 dark:bg-background-dark rounded-xl">
                        <p class="text-xs text-text-secondary-light">Pays d'Origine</p>
                        <p class="font-bold"><?= ($info_animal->getPaysOrigine()) ?></p>
                    </div>
                    <div class="p-3 bg-gray-50 dark:bg-background-dark rounded-xl">
                        <p class="text-xs text-text-secondary-light">Habitat Actuel</p>
                        <p class="font-bold"><?php
                                                $habitat_actuel = new Habitat();
                                                $habitat_actuel->getHabitat($info_animal->getIdHabitat());
                                                echo $habitat_actuel->getNomHabitat();
                                                ?></p>
                    </div>
                </div>
        ...
        <div id="modalEditAnimal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div class="bg-surface-light dark:bg-surface-dark w-full max-w-md max-h-[90vh] overflow-y-auto p-6 rounded-xl shadow-2xl border border-blue-500/30">
                ...
                <form action="php/update_animal.php" method="POST" class="flex flex-col gap-4">
                    <input type="hidden" name="id_animal" value="<?= $info_animal->getIdAnimal() ?>">

                    <div>
                        <label class="block text-sm font-medium mb-1">Nom de l'animal</label>
                        <input type="text" name="nom_animal" value="<?= ($info_animal->getNomAnimal()) ?>" required
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="espece" value="<?= ($info_animal->getEspeceAnimal()) ?>" required
                                class="w-full rounded-lg border-gray-300 dark:bg-background-dark dark:border-gray-700 focus:ring-blue-500 text-sm">
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="pays_origine" value="<?= ($info_animal->getPaysOrigine()) ?>" required
                            class="w-full rounded-lg border-gray-300 dark:bg-background-dark dark:border-gray-700 focus:ring-blue-500 text-sm">
                    </div>

                    <div>
                        <label class="block text-sm font-medium mb-1">URL de l'image</label>
                        <input type="url" name="image_url" value="<?= ($info_animal->getImageUrl()) ?>" required
                            class="w-full rounded-lg border-gray-300 dark:bg-background-dark dark:border-gray-700 focus:ring-blue-500 text-sm">
                    </div>
                    ...
                    <div>
                        ...
                        <select name="id_habitat" required class="w-full rounded-lg border-gray-300 dark:bg-background-dark dark:border-gray-700 text-sm">
                            ...
                            foreach ($array_habitats as $habitat) : ?>
                                <option value="<?=  $habitat->getIdHabitat() ?>" <?= ($info_animal->getIdHabitat() == $habitat->getIdHabitat()) ? 'selected' : '' ?>>
                    ...
                    <div>
                        ...
                        <textarea name="description_animal" rows="4" required
                            class="w-full rounded-lg border-gray-300 dark:bg-background-dark dark:border-gray-700 focus:ring-blue-500 text-sm"><?= ($info_animal->getDescriptionAnimal()) ?></textarea>
                    </div>
